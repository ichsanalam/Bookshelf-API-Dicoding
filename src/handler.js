...
const getBooksById = (request, h) => {
  // mengambil bookId di parameter url
  const { bookId } = request.params;

  // cari buku dengan id yg diminta
...
const editBooksById = (request, h) => {
  ...
  // mengambil request body yg di request oleh client
  const { name, year, author, summary, publisher, pageCount, readPage, reading } = request.payload;
  ...
  // jika buku ada, maka salin list buku berdasarkan index yg dicari menggunakan spread operators lalu update berdasarkan request.payload
  if (index !== -1) {
    books[index] = {
      ...books[index],
      name,
...
module.exports = { saveBooks, getBooks, getBooksById, editBooksById, deleteBookbyId };
